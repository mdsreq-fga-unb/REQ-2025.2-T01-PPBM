<script lang="ts">
    ...
    $: stats = {
        ...
        totalAlunos: turmasData.reduce(
            (acc, turma) => acc + (turma.alunos_count || 0),
            0,
        ),
        totalDocentes: turmasData.reduce(
            (acc, turma) => acc + (turma.docentes_count || 0),
            0,
        ),
        totalUnidades: new Set(
            turmasData.map((t) => t.unidade_turma).filter(Boolean),
        ).size,
    };
    ...
    const columns: Column[] = [
        ...
        },
        {
            ...
            sortable: true,
            render: (row: Turma) => {
                const count = row.docentes_count || 0;
            ...
            render: (row: Turma) => [
                ...
                },
                {
                    ...
                    props: {
                        ...
                        text: "Remover",
                        onClick: () =>
                            removerTurma(row.id_turma, row.nome_turma),
                    },
                },
            ],
        },
        ...
        filteredTurmas = turmasData.filter((turma) => {
            ...
            const matchUnidade =
                ...
                (turma.unidade_turma &&
                    turma.unidade_turma
                        .toLowerCase()
                        .includes(unidadeFilter.toLowerCase()));
    ...
    function editarTurma(id: number) {
        // Navigate to cadastrar-turmas page with id for edit mode
        window.location.href = `/admin/cadastrar-turmas?id=${id}`;
    }
        ...
        confirmDialogAction = async () => {
            ...
            try {
                ...
                displayToast(
                    error instanceof Error
                        ? error.message
                        : "Erro ao remover turma. Por favor, tente novamente.",
                    "error",
                );
            } finally {
    ...
    function exportarLista() {
        ...
        const dados = filteredTurmas.map((turma) => ({
            ...
            Alunos: turma.alunos_count || 0,
            Docentes: turma.docentes_count || 0,
        }));
        ...
        link.setAttribute(
            "download",
            `turmas_${new Date().toISOString().split("T")[0]}.csv`,
        );
...
<div class="turmas-manager">
    ...
    <div class="stats-row">
        ...
        <div class="stat-card tertiary">
            <div class="stat-value">{stats.totalDocentes}</div>
